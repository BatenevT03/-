...
vector<vector<PointData>> gauss_points_per_element;  // [номер_элемента][точки_Гаусса]
vector<vector<vector<double>>> phi_cache_global;
...
double basic_func(int i,double x, double y, double z)
{
	double X[2] = { 1 - x,x };
	double Y[2] = { 1 - y,y };
	double Z[2] = { 1 - z,z };
...
double Gauss2(const vector<double>& x, const vector<double>& y, const vector<double>& z,
	int i, int j, int c, double a, double b,int numfe) {

	
	
	static vector<PointData> local_gauss_points; // Локальные точки Гаусса
	static bool first_call = true;

	// Инициализация при первом вызове
	if (first_call) {
		phi_cache_global.resize(27);
		local_gauss_points.resize(27);
		first_call = false;
	}
	int gauss_index = 0;
		...
		for (int gj = 0; gj < 3; ++gj) {
			...
			for (int gk = 0; gk < 3; ++gk) {
				...
				if (i == 0 && j == 0) 
				{
					...
					{
						double phi = basic_func(node, xi, eta, theta);
						x_global += x[node] * phi;
						y_global += y[node] * phi;
						z_global += z[node] * phi;
					}
					// Сохраняем точку
					local_gauss_points[gauss_index] = { x_global, y_global, z_global, 0.0 };

					// Кэшируем базисные функции
					for (int node = 0; node < 8; ++node) {
						phi_cache_global[numfe][gauss_index][node] = basic_func(node, xi, eta, theta);
					}
				}
	...
	// Сохраняем все точки Гаусса элемента при завершении его обработки
	if (i == 7 && j == 7) {  // Последний вызов для элемента
		gauss_points_per_element.insert(gauss_points_per_element.end(),
			local_gauss_points.begin(),
			local_gauss_points.end());
	}
...
void GLOBAL_MATRIX::UpdateGaussPointValues(vector<double>& solution) {
	int point_counter = 0;
	for (int elem = 0; elem < local::count; ++elem) {
		for (int gp = 0; gp < 27; ++gp) {
			double value = 0.0;
			for (int node = 0; node < 8; ++node) {
				int global_node = matr[elem].mas[node] - 1;
				value += solution[global_node] * phi_cache_global[elem][gp][node];
			}
			gauss_points_per_element[elem][point_counter++].value = value;
			cout << gauss_points_per_element[elem][point_counter++].x << " " << gauss_points_per_element[elem][point_counter++].y << " " << gauss_points_per_element[elem][point_counter++].z << " Value: " << gauss_points_per_element[elem][point_counter++].value << endl;
		}
	}
}
...
void GLOBAL_MATRIX::formier_matrix() {//шестигранники
	...
	for (int k = 0; k < local::count; k++) {
		...
		// Цикл по предвычисленным точкам Гаусса
			...
			for (int i = 0; i < 8; ++i) {
				for (int j = 0; j < 8; ++j) {
					Ke[i][j] = matr[k].lambda*(Gauss2(x, y, z, i, j, 1, 0, 1,k) +
						Gauss2(x, y, z, i, j, 2, 0, 1,k) +
						Gauss2(x, y, z, i, j, 3, 0, 1,k));
					//cout << Ke[i][j] << " ";
				}
				//cout << endl;
			}
