...
void GLOBAL_MATRIX::read_local(string file)
{
	int size;
	ifstream in(file);
	//in >> local::count;
	...
	cout << "Finite elements: " << size << endl;
	...
	//ofstream pizduk("localfe.txt");
	//for (i = 99000;i < matr.size();i++)
	//{
	//	//if (set[matr[i].mas[j] - 1].mas[0])
	//	pizduk << i << endl;
	//	for (int j = 0;j < 8;j++)
	//	{
	//		
	//		pizduk << set[matr[i].mas[j] - 1].mas[0] << " " << set[matr[i].mas[j] - 1].mas[1] << " " << set[matr[i].mas[j] - 1].mas[2] << endl;
	//	}
	//	pizduk << endl;
	//}
...
void GLOBAL_MATRIX::read_cross(string file)
{
	//set == point
...
void GLOBAL_MATRIX::formier_profil() {
	...
	for (int ielem = 0; ielem < matr.size(); ++ielem) {
		int numUnknowns = 8;
...
//void GLOBAL_MATRIX::Ugauss(int num)
//{
//	double res=0.;
//	for (int gi = 0; gi < 3; ++gi) {
...
//				for (int i = 0;i < 8;i++)
//				{
//					res += x[matr[num].mas[i]-1]*basic_func(i, xi, eta, theta);
//				}
//				cout << "Point: "<< xi <<" " << eta<< " "<<theta<<" "<<"Value: " << res << endl;
//			}
//		}
//	}
//}

int is_on_plane(double xa, double ya, double za, double xb, double yb, double zb, double xc, double yc, double zc,double x, double y, double z)
{
	double positive = (x - xa) * (yb - ya) * (zc - za) +
		(xb - xa) * (yc - ya) * (z - za)+
		(xc-xa)*(y-ya)*(zb-za);
	double negaive=(xc-xa)*(yb-ya)*(z-za)+
		(xb-xa)*(y-ya)*(zc-za)+
		(x-xa)*(yc-ya)*(zb-za);
	return positive - negaive;
}

int GLOBAL_MATRIX::find6el(double x1, double y1, double z1)
{
	int i;
	...
	for (i = 0;i < matr.size();i++)
	{
		...
		vector<double> x, y, z;
		double xmin = 1e60, xmax = -1e60, ymin = 1e60, ymax = -1e60, zmin = 1e60, zmax = -1e60;
		for (int j = 0;j < 8;j++)
		{
			x.push_back(set[matr[i].mas[j] - 1].mas[0]);
			if (x[j] < xmin) xmin = x[j];
			if (x[j] > xmax) xmax = x[j];
			y.push_back(set[matr[i].mas[j] - 1].mas[1]);
			if (y[j] < ymin) ymin = y[j];
			if (y[j] > ymax) ymax = y[j];
			z.push_back(set[matr[i].mas[j] - 1].mas[2]);
			if (z[j] < zmin) zmin = z[j];
			if (z[j] > zmax) zmax = z[j];
		}
		if (x1<xmin || x1>xmax || y1<ymin || y1>ymax || z1<zmin || z1>zmax) 	continue;



		// 3. Проверка принадлежности выпуклому шестиграннику
		bool inside = true;

		// Грани шестигранника (номера вершин в порядке обхода против часовой стрелки)
		vector<vector<int>> faces = {
			{0, 1, 2, 3}, // Нижняя грань
			{2, 3, 6, 7}, // Задняя грань
			{0, 2, 4, 6},  // Левая грань
			{4, 5, 6, 7}, // Верхняя грань
			{0, 1, 4, 5}, // Передняя грань
		...
		{
			if (is_on_plane(x[faces[i][0]], y[faces[i][0]], z[faces[i][0]],
				x[faces[i][1]], y[faces[i][1]], z[faces[i][1]],
				x[faces[i][2]], y[faces[i][2]], z[faces[i][2]],
				x1, y1, z1) >= 0.) continue;//cout << "good on gran: " << i + 1 << endl;
			
			else flag = false;

			//{ cout << "bad on gran: " << i + 1 << endl; flag = false; }
		}


		if (is_on_plane(x[faces[3][0]], y[faces[3][0]], zmax,
			x[faces[3][1]], y[faces[3][1]], zmax,
			x[faces[3][2]], y[faces[3][2]], zmax,
			x1, y1, z1) > 0.)
			flag = false;
...
double phi(double xi, double eta, double theta, int i)
{
	double x[] = { 1 - xi,xi };
	double y[] = { 1 - eta,eta };
	double z[] = { 1 - theta,theta };
	switch (i)
	{
...
bool solveLinearSystem(const vector<vector<double>>& A, const vector<double>& f, vector<double>& res) {
	const int n = A.size();
	if (n == 0 || A[0].size() != n || f.size() != n) {
		return false;
	}
...
void GLOBAL_MATRIX::Method_Newton(int nf1,vector<double>&a,bool sign)
{
	//Метод Ньютона для определения координат на единичном кубе, хотим найти xi,eta,theta
	int  residual, flag = true;
	double xi = 0., eta = 0., theta = 0.;//начальное приближение

	while (flag)
	{

		vector<vector<double>> A(3, vector<double>(3, 0));
		vector<double> F(3, 0), res(3, 0);
		...
		for (int i = 0;i < 3;i++)
		{
			...
			for (int k = 0;k < 8;k++)
			{
				A[i][0] += set[matr[nf1].mas[k] - 1].mas[i] * dphidxi(eta, theta, k);
				A[i][1] += set[matr[nf1].mas[k] - 1].mas[i] * dphideta(xi, theta, k);
				A[i][2] += set[matr[nf1].mas[k] - 1].mas[i] * dphidtheta(xi, eta, k);
				F[i] -= set[matr[nf1].mas[k] - 1].mas[i] * phi(xi, eta, theta, k);

			}
			F[i] += a[i];

		}
		//СЛАУ собрали, теперь решить надо
		solveLinearSystem(A, F, res);
		xi += res[0];
		eta += res[1];
		theta += res[2];
		residual = sqrt(F[0] * F[0] + F[1] * F[1] + F[2] * F[2]);
		if (residual < 1e-13) flag = false;
	}
	cout << "Point in Finite Element with coordintaes:\n";
	cout << "local coordinates: " << xi << " " << eta << " " << theta << endl;
	for (int i = 0;i < 8;i++)
	{
		cout << set[matr[nf1].mas[i] - 1].mas[0] << " " << set[matr[nf1].mas[i] - 1].mas[1] << " " << set[matr[nf1].mas[i] - 1].mas[2]<<" " <<  phi(xi, eta, theta, i) << endl;
		f[matr[nf1].mas[i] - 1] = phi(xi, eta, theta, i);
		if (!sign) f[matr[nf1].mas[i] - 1] *= -1;
	}
}


void GLOBAL_MATRIX::SetF()
{
	...
	//in >> c[0] >> c[1] >> c[2];
	in.close();
	//a,b,c это точки, где есть точечный источник
	nf1 = find6el(a[0], a[1], a[2]);//номера КЭ, где есть точечный источник
	cout << "Electrode 1 in Finite Element: " << nf1 << endl;
	nf2 = find6el(b[0], b[1], b[2]);//номера КЭ, где есть точечный источник
	cout << "Electrode 2 in Finite Element: " << nf2 << endl;
	//nf3 = find6el(c[0], c[1], c[2]);//номера КЭ, где есть точечный источник
	
	if (nf1!=-1)
		Method_Newton(nf1, a,false);
		//Method_Newton(nf1, a, true);
	if (nf2 != -1)
		Method_Newton(nf2, b,true);
	if (nf3 != -1)
		Method_Newton(nf3, c,false);
...
double GLOBAL_MATRIX::U6(double nx, double y, double z)
{
	int nf1 = find6el(nx, y, z);

	double xi, eta, theta;
	vector<double>a = { nx,y,z };
	if (nf1 != -1)
	{
		int  residual, flag = true;
		xi = 0., eta = 0., theta = 0.;//начальное приближение
	...
	{
		cout << "Not found point in Finite Elements: " << nx << " " << y << " " << z << endl;
		return 0;
	}
	...
	//f(x0)=sum(q_i*psi_i(x0))
	for (i = 0; i < 8; i++)
		sum += x[matr[nf1].mas[i] - 1] * phi(xi,eta,theta,i);
		//sum += x[matr[nf1].mas[i] - 1] * get_psi(lc - 1, i, nx, y, z);
	return sum;
}
