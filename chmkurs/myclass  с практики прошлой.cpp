...
//	jg.resize(s);// = new int[s];
//	gg.resize(s);// = new double[s];
//	ig[0]=1;
//	ig[1]=1;
//	for(int i=0,d=0;i<n-1;i++) {
//		int k=0;//количество ненулевых элементов в i-ой строке профил¤
//		for(int j=0;j<=i;j++)
//			if(prof[i][j]==1) {
//				jg[d]=j+1;
//				d++;
...
//void GLOBAL_MATRIX::unregular()
//{
//	int hx = 0, hy = 0, hz = 0;
//	double x0 = 0, x1 = 2, h = 1,step=0,hu=1.2;
//	ofstream in("cross.txt");
//	//cin >> x0 >> x1 >> h;
//	double l = pow(((x1 - x0) / h), 3);//число КЭ
//	double n = pow(((x1 - x0) / h) + 1, 3);//число узлов
...
//	in << n << endl << endl;
//
//	int stepx, stepy, stepz=0;
//	for (i = 0;i < nx;i++,stepz++)
//	{
//		stepy = 0;
//		for (j = 0;j < nx;j++,stepy++)
//		{
//			stepx = 0;
//			for (k = 0;k < nx;k++,stepx++)
//			{
//				//in << k * pow(hu,stepx) << " " << j * pow(hu, stepy) << " " << i * pow(hu, stepz) << endl;
//				in << k/nx * pow(hu, stepx) << " " << j/nx * pow(hu, stepy) << " " << i/nx * pow(hu, stepz) << endl;
//			}
//		}
//	}
//	in.close();
...
//}
//
//void GLOBAL_MATRIX::generate_2kr()
//{
//	ofstream in("kraev_2.txt");
...
//	//нижняя грань
//	int temp = 1, i, j, k;
//	double lx = pow(local::count, 1 / 3.), nx = lx + 1;
//	lx = nx - 1;
//	for (i = 0;i < lx;i++, temp++)
//		for (j = 0;j < lx;j++, temp++)
//		{
...
//			d = temp + nx + 1;
//			//in << a << " " << b << " " << c << " " << d << endl;
//			//in << -dudn(set[a-1].mas,3) << " " << -dudn(set[b-1].mas,3) << " " << -dudn(set[c-1].mas,3) << " " << -dudn(set[d-1].mas,3) << endl;
//			in2 << a << " " << ug(set[a - 1].mas) << endl;
//			in2 << b << " " << ug(set[b - 1].mas) << endl;
//			in2 << c << " " << ug(set[c - 1].mas) << endl;
//			in2 << d << " " << ug(set[d - 1].mas) << endl;
//		}
//	in << endl;
//	//левая грань
//	temp = 1;
//	for (i = 0;i < lx;i++, temp += nx)
//		for (j = 0;j < lx;j++, temp += nx)
//		{
//			a = temp;
//			b = temp + nx;
//			c = temp + nx * nx;
//			d = temp + nx * nx + nx;
//			//in << a << " " << b << " " << c << " " << d << endl;
//			//in << -dudn(set[a-1].mas,1) << " " << -dudn(set[b-1].mas,1) << " " << -dudn(set[c-1].mas,1) << " " << -dudn(set[d-1].mas,1) << endl;
//			in2 << a << " " << ug(set[a - 1].mas) << endl;
//			in2 << b << " " << ug(set[b - 1].mas) << endl;
//			in2 << c << " " << ug(set[c - 1].mas) << endl;
//			in2 << d << " " << ug(set[d - 1].mas) << endl;
...
//			//cout << a << " " << b << " " << c << " " << d << endl;
//			//in << a << " " << b << " " << c << " " << d << endl;
//			//in << -dudn(set[a-1].mas,2) << " " << -dudn(set[b-1].mas,2) << " " << -dudn(set[c-1].mas,2) << " " << -dudn(set[d-1].mas,2) << endl;
//			in2 << a << " " << ug(set[a - 1].mas) << endl;
//			in2 << b << " " << ug(set[b - 1].mas) << endl;
//			in2 << c << " " << ug(set[c - 1].mas) << endl;
//			in2 << d << " " << ug(set[d - 1].mas) << endl;
...
//	//задняя грань
//	temp = 1 + lx * nx;
//	for (i = 0;i < lx;i++, temp = 1 + lx * nx + nx * nx * i)
//		for (j = 0;j < lx;j++, temp++)
//		{
//			a = temp;
//			b = temp + 1;
